...
DEFAULT_DATA_FILE = "train_dataset.csv"
DEFAULT_BATTERY_ID = "B5"
DEFAULT_TARGET_COL = "SOH"
DEFAULT_ORDER = (1, 1, 1)
DEFAULT_SEASONAL_ORDER = (1, 0, 1, 10)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run a SARIMA experiment on one battery.")
    parser.add_argument(
        "--data-file",
        default=DEFAULT_DATA_FILE,
        help="CSV file name inside data/processed, for example train_dataset.csv or test_dataset.csv.",
    )
    parser.add_argument(
        "--battery-id",
        default=DEFAULT_BATTERY_ID,
    ...
    parser.add_argument(
        "--order",
        default="1,1,1",
        help="Non-seasonal SARIMA order as p,d,q.",
    )
    parser.add_argument(
        "--seasonal-order",
        default="1,0,1,10",
        help="Seasonal SARIMA order as P,D,Q,m.",
    )
    parser.add_argument(
        "--split-ratio",
        type=float,
...
def main():
    ...
    seasonal_order = parse_tuple(args.seasonal_order, 4, "--seasonal-order")
    ...
    plt.title(f"SARIMA Forecast: {args.battery_id} train/test split")
    plt.xlabel("Cycle")
    plt.ylabel(args.target_col)
    plt.legend()
    plt.grid(True, alpha=0.3)
